...
// Get image dimensions endpoint (pure Node.js — no Python needed)
router.post('/get-dimensions', processLimiter, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        res.status(500).send('Failed to get dimensions.');
    }
});
...
// Resize endpoint (shrink/resize by pixels or percentage — pure Node.js with sharp)
router.post('/resize', processLimiter, queueMiddleware, optionalAuth, upload.single('image'), async (req, res) => {
    if (!req.file) return res.status(400).send('No file uploaded.');
    ...
    const resizeType = req.body.resizeType || 'percentage';
    const percentage = parseInt(req.body.percentage) || 100;
    const targetWidth = parseInt(req.body.width) || 800;
    const targetHeight = parseInt(req.body.height) || 600;
    const maintainAspect = req.body.maintainAspect !== 'false';
    const quality = parseInt(req.body.quality) || 90;

    console.log('Resize options:', { resizeType, percentage, targetWidth, targetHeight, maintainAspect, quality });
    ...
    try {
        ...
        if (resizeType === 'percentage') {
            newW = Math.round(originalW * percentage / 100);
            newH = Math.round(originalH * percentage / 100);
        } else {
            newW = targetWidth;
            newH = targetHeight;
            if (maintainAspect) {
        ...
        let pipeline = sharp(inputPath).resize(newW, newH, { fit: 'fill' });
