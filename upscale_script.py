# upscale_script.py
# (Only used for resizing natively in Python now. Upscaling has moved to the gpu-provider module).
...
mode = sys.argv[3]  # 'resize' (upscale is deprecated here)
...
try:
    if mode == 'resize':
        ...
        print(f"Unknown mode: {mode} (Upscaling has been migrated to gpu-provider module)", file=sys.stderr)
        sys.exit(1)
